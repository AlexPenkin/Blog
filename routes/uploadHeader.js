...
app.app.route('/uploadHeader')
  .post(function (req, res, err) {
    ...
    pro.then(resp => {
        ...
        app.mongo.Blog.update({
          title: `${decodeURIComponent(req.headers.title)}`
        }, {
          $set: {
            headImg: `/posts/${decodeURIComponent(req.headers.autor)}/${decodeURIComponent(req.headers.title)}/header${path.extname(decodeURIComponent(req.headers.name))}`
          }
        }, function (err, a) {
          console.log(a);
        })
